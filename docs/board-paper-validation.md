...
both feature configurations. The all-feature suite passed 4,584 root and 491
configurator tests; no-default-features passed 4,424 root and 490 configurator
...
The visible outer palette edits that same draft; its clicks preserve pending
pattern and spacing changes, and both palette and sheet are marked for repaint.
...
[Initial consumer measurements](board-paper-consumer-performance.csv) use the actual
...
not construct the paper again. Those initial warm measurements covered only
`ensure` and `blit`, so they did not detect redundant painting in the live caller.
The benchmark now invokes the production committed-canvas dispatcher, including
backdrop preparation, and asserts that a cache hit constructs no paper source.
A 4K regression checks that path and paper restoration after a failed blit.
The historical CSV below has not been remeasured with this expanded harness.
