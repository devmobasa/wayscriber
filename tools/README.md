...
- **check-cargo-lanes.py** - Guard the configurator feature graph and the Cargo entry points
  ...
  - Raw-command detection tokenizes each line, splits it on shell operators, and reads past control words, environment assignments, wrapper programs and their option operands, so `if command cargo ...`, `env -u NAME cargo ...`, `sudo -u USER cargo ...`, `timeout -k 10 300 cargo ...`, and `/usr/bin/cargo ...` are all raw Cargo commands. Command strings passed to a shell's `-c` option are scanned recursively too. The same normalization counts the allowlisted commands, so an allowlisted command that gains a prefix still counts exactly once
  - A Python entry point that consumes the manifest through the loader must import `cargo_lanes` and read the routed consumer's `.operations` from `load_manifest().consumer(...)`; an unused import and a surviving label cannot hide inlined vectors
  - Schema: a lane owns package and feature selection, so an operation's argv may not add `-p`/`--package`, `-F`/`--features`, `--workspace`, `--exclude`, `--all-features`, or `--no-default-features` after the lane arguments, including attached short values, equals spellings, and either side of a bare `--`
...
- **fixtures/cargo-lanes/** - Inputs for the `check-cargo-lanes.py` self-test
  ...
  - `entry-point-cases.json` and `entry-points/` hold workflow texts for one complete case and the broken ones (a deleted linkage step, a reintroduced raw command, a duplicated driver call, a missing Arch gate, an undeclared consumer, and raw Cargo commands spelled plainly, environment-prefixed, wrapped, path-qualified, control-prefixed, or passed through a shell evaluator), plus checker texts for the loader entry point with real manifest consumption, with the loader gone, with only its name left, and with an unused import beside inlined vectors
  - `manifest-cases.json` and `manifests/` hold whole manifests for the schema rules: the smallest one the loader accepts, and three whose operation argv selects its own package or feature after the lane arguments, including Cargo's short `-F` alias
...
- **check-aur-templates.py** - Guard the external configurator AUR recipe
  - Renders the checked-in templates with fixture values and asserts: no unresolved token, every configurator build command passes `--features adw-modern` and no other build command does, `depends` contains `libadwaita>=1.7`, `gtk4`, and `libxkbcommon`, `makedepends` contains `cargo`, the `.SRCINFO` structure is well formed, and the two files agree on every field `.SRCINFO` can express
  ...
  - `--self-test` replays the fixtures for the rules a healthy tree cannot exercise: a throwaway repository whose tracked template pair is ignored because the `.gitignore` negation chain was deleted, a rendered `.SRCINFO` with a package section repeated, two Cargo builds joined on one line with the modern feature on the wrong one, and a baseline configurator rebuild after a modern build. Each must be rejected, and the ignore fixture also asserts that the default `git check-ignore` still accepts it, since that disagreement is what `--no-index` exists for
