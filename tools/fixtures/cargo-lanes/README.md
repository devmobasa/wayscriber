...
- `entry-point-cases.json` — one contract-complete workflow plus five texts that
  each break one entry-point rule: a deleted linkage step, a reintroduced raw
  command, a duplicated driver call, a missing Arch gate, and a raw Cargo
  command that is neither a lane operation nor allowlisted.
