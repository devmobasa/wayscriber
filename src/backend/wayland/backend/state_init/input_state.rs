...
use crate::config::{Action, Config, KeybindingsConfig, QuickColorPalette, ShortcutTrigger};
...
fn build_action_map(config: &Config) -> HashMap<ShortcutTrigger, Action> {
...
fn build_action_bindings(config: &Config) -> HashMap<Action, Vec<ShortcutTrigger>> {
