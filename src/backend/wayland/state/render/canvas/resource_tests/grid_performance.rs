                ...
                for spacing in [8, 40] {
                    for count in [0, 20, 200] {
                        ...
                        for sample in 0..6 {
                            ...
                            let start = Instant::now();
                            assert!(layer.ensure(&measurer, &mut caches, &frame.shapes, request));
                            let geometry = FrameGeometry::new(size.0, size.1, 1);
                            let canvas_frame = CanvasFrame {
                                draw_committed: true,
                                render_transients: false,
                                transform_active: true,
                                origin: request.origin,
                                zoom_scale: None,
                                text_halo_enabled: request.text_halo_enabled,
                            ...
                            let mut backdrop =
                                background::CanvasEraserContext::for_board(request.background);
                            ...
                            .unwrap();
                            assert!(perf.canvas_layer_cache_used);
                            assert!(backdrop.replay_context().pattern.is_none());
