...
pub(super) fn draw_thickness_section(layout: &mut SidePaletteLayout, y: &mut f64) {
        ...
        if snapshot.side_section_collapsed(ToolbarSideSection::Thickness) {
            *y += slider_card_h + section_gap;
            ...
            let minus_hover = hover
                .map(|(hx, hy)| {
                    point_in_rect(hx, hy, minus_x, thickness_slider_row_y, btn_size, btn_size)
                })
                .unwrap_or(false);
            draw_button(
                ctx,
            ...
            let plus_hover = hover
                .map(|(hx, hy)| {
                    point_in_rect(hx, hy, plus_x, thickness_slider_row_y, btn_size, btn_size)
                })
                .unwrap_or(false);
            draw_button(
                ctx,
    ...
    {
        let eraser_card_h = layout.spec.side_eraser_mode_height(snapshot);
