...
pub(super) fn draw_header(layout: &mut SidePaletteLayout) -> f64 {
    ...
    let display_label = ellipsize_to_width(ctx, &label, label_w);
    draw_label_left(ctx, label_x, chip_y, label_w, chip_h, &display_label);
    hits.push(HitRegion {
...
fn ellipsize_to_width(ctx: &cairo::Context, text: &str, max_width: f64) -> String {
    if max_width <= 0.0 {
        return String::new();
    }
    if ctx.text_extents(text).map(|ext| ext.width()).unwrap_or(0.0) <= max_width {
