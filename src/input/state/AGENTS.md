...
- `core/` owns board state, history, selection, panels, properties, command palette, board picker, utilities, and session preflight. Panels with their own lifecycle own a state type (`core/font_picker/state.rs`, `core/command_palette/state.rs`, `core/help_overlay/state.rs`, `core/board_picker/panel.rs`, `core/menus/context_menu.rs`, `core/color_picker_popup/panel.rs`,
  `core/radial_menu/panel.rs`, `core/status_hud/state.rs`); `core/base/ui_visibility.rs` groups the UI visibility preferences; `core/search.rs` holds the shared fuzzy scorer.
