...
- `core/` owns board state, history, selection, panels, properties, command palette, board picker, utilities, and session preflight. Panels with their own lifecycle own a state type (`core/font_picker/state.rs`, `core/command_palette/state.rs`, `core/help_overlay/state.rs`, `core/board_picker/panel.rs`, `core/menus/context_menu.rs`, `core/color_picker_popup/panel.rs`, `core/radial_menu/panel.rs`, `core/status_hud/state.rs`, `core/zoom_chip/state.rs`, `core/properties/state.rs`, `core/tour.rs::TourState`); code outside `input::state` reads them through accessors, not fields. Shared modal keyboard-repeat timing belongs to `core/key_repeat.rs`, not to an individual panel's input handler. `core/style.rs` owns drawing-style mutation and preset/session conversion, `core/presets.rs` owns preset slot lifecycle, and `core/history_limits.rs` owns undo retention and delayed playback scheduling. `core/text_editing.rs` owns text mode, asynchronous edit identity, IME composition, caret/selection edits, text-block pointer state, and existing-shape edit lifecycle; root wrappers retain dirty tracking, redraw, session, and backend-effect coordination. `core/text_placement.rs` keeps the live text/note draft inside the visible canvas. `core/recognition_chip.rs::RecognitionFeedback` owns the transient Shape Pen recognition chip and its enable switch; it is never stored in frames or sessions. `core/selection.rs` owns selection membership, nudge-axis memory, and polygon click timing; `core/selection/clipboard.rs` owns local shape clipboard generations, publication state, paste request identity, and image-save fallback. `core/keymap.rs` owns action and sequence matching, rebind revisions, pointer-button consumption, drag-tool bindings, active pointer-drag identity, and shortcut capture; shared modifiers remain on `InputState`. `core/view.rs`, `core/pointer.rs`, and `core/index.rs` own view transforms, pointer bookkeeping, and canvas hit-test/index policy respectively, while `spotlight.rs::SpotlightWheelGesture` owns wheel-burst state; root wrappers retain cross-owner dirty, redraw, and board coordination. `core/toolbar/state.rs` owns toolbar visibility, display preferences, resolved layout and item overrides, customization drag/drawers, and top-strip menu state; root wrappers retain redraw, toast, focus-mode, persistence, and `UiVisibility` mirror coordination. `core/feedback.rs` owns active and queued toasts, toast hit geometry and press identity, blocked-action animation, capability-warning memory, and feedback duration policy; root wrappers retain redraw and dispatched-action coordination. `core/base/ui_visibility.rs` groups the remaining UI visibility preferences; `core/search.rs` holds the shared fuzzy scorer.
- `core/modes.rs::ChromeModes` owns presenter, focus, and light-mode flags, configuration, restore snapshots, and transition data operations; root coordination retains cross-owner writes and feedback.
- `core/session_flags.rs::SessionFlags` owns session dirtiness, preflight options, pending save-as overwrite state, and the last capture path; the redraw latch remains on `InputState`.
- `core/board/transitions.rs::BoardTransitions` owns board-switch color restoration, bounded recents, delete confirmations, and timed board/page restore queues; `BoardManager` remains the persistence-facing board owner.
...
- Keep toast queueing, promotion, rate limiting, geometry, press/release identity, blocked-action timing, and capability-warning memory on `Feedback`; root wrappers add redraw and action-dispatch effects.
- Keep mode restore snapshots on `ChromeModes`, and preserve mutual exclusion plus exact presenter/focus/light restoration without changing persisted toolbar pins.
- Keep session flags and paths on `SessionFlags`, but keep redraw scheduling on the root; rollback restores dirtiness without replacing path state.
- Keep recents, previous-color restoration, delete windows, and timed undo queues on `BoardTransitions`; root board actions retain `BoardManager` mutation, toasts, effects, and redraw/session coordination.
