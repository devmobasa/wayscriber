...
mod tests {
    ...
    fn tour_copy_tracks_rebound_shortcuts() {
        use crate::config::{KeybindingsConfig, ShortcutTrigger};
        ...
        bindings.insert(
            ...
            vec![ShortcutTrigger::parse("Ctrl+Shift+P").expect("binding")],
        );
        let mut state = make_test_input_state();
    ...
    fn status_bar_step_teaches_board_picker_and_tracks_binding() {
        ...
        bindings.insert(
            Action::BoardPicker,
            vec![ShortcutTrigger::parse("Ctrl+Shift+B").expect("binding")],
        );
        let mut state = make_test_input_state();
        state.set_action_bindings(bindings);
