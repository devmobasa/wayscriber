...
impl InputState {
    pub(super) fn build_selection_property_entries(
        ...
        if color_summary.applicable {
            let value = summary_value(&color_summary, color_label);
        ...
        if thickness_summary.applicable {
            let value = summary_value(&thickness_summary, |v| format!("{v:.1}px"));
            entries.push(SelectionPropertyEntry {
        ...
        if fill_summary.applicable {
            let value = summary_value(&fill_summary, |v| if v { "On" } else { "Off" }.to_string());
            entries.push(SelectionPropertyEntry {
        ...
        if font_summary.applicable {
            let value = summary_value(&font_summary, |v| format!("{v:.0}pt"));
            entries.push(SelectionPropertyEntry {
        ...
        if head_summary.applicable {
            let value = summary_value(&head_summary, |v| {
                if v { "End" } else { "Start" }.to_string()
            });
            entries.push(SelectionPropertyEntry {
        ...
        if angle_summary.applicable {
            let value = summary_value(&angle_summary, |v| format!("{v:.0} deg"));
            entries.push(SelectionPropertyEntry {
        ...
        if text_bg_summary.applicable {
            let value = summary_value(&text_bg_summary, |v| {
