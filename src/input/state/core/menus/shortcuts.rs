...
mod tests {
    use super::*;
    use crate::config::{RadialMenuMouseBinding, ShortcutTrigger};
    use crate::input::state::test_support::make_test_input_state_with_action_bindings;
    use std::collections::HashMap;

    fn binding_map(entries: &[(Action, &[&str])]) -> HashMap<Action, Vec<ShortcutTrigger>> {
        entries
            .iter()
            .map(|(action, values)| {
                (
                    *action,
                    values
                        .iter()
                        .map(|value| ShortcutTrigger::parse(value).expect("binding"))
                        .collect(),
                )
            })
            .collect()
