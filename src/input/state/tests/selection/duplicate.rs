...
fn shape_bounds_center(shape: &Shape) -> (i32, i32) {
    let bounds = shape.bounding_box().expect("shape bounds");
...
fn copy_paste_selection_centers_shape_at_pointer() {
    ...
    assert_eq!(shape_bounds_center(&pasted.shape), request.anchor.point());
}
...
fn repeated_paste_selection_uses_current_pointer_anchor() {
    ...
    let centers = frame
        .shapes
        .iter()
        .map(|shape| shape_bounds_center(&shape.shape))
        .collect::<Vec<_>>();
    assert_eq!(centers[1..], [(100, 120), (200, 220)]);
}
