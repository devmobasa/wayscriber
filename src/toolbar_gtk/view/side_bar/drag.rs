...
impl SideBar {
    /// Keep the input surface parked and read stable start-relative motion on
    /// compositor ticks while the root-owned state resolves lifecycle intents.
    pub(super) fn attach_move_drag(&mut self, grip: &gtk4::DrawingArea) {
        ...
        drag.connect_drag_begin(move |gesture, start_x, start_y| {
            ...
            if !grip
                ...
                || !begin_intents.send(ViewIntent::Side(DragIntent::Begin { controller }))
            {
                gesture.set_state(gtk4::EventSequenceState::Denied);
            }
        });
        ...
        drag.connect_drag_end(move |_, dx, dy| {
            crate::toolbar_gtk::drag_debug_log(format!(
                "side raw end controller={controller} delta=({dx:.3},{dy:.3})"
            ));
            let _ = end_intents.send(ViewIntent::Side(DragIntent::End { controller, dx, dy }));
        });

        let cancel_intents = self.intents.clone();
        drag.connect_cancel(move |_, _| {
            let _ = cancel_intents.send(ViewIntent::Side(DragIntent::Cancel { controller }));
        });
    ...
    pub(in crate::toolbar_gtk::view) async fn handle_drag_intent(
        ...
        match intent {
            DragIntent::Begin { controller } => {
                ...
                crate::toolbar_gtk::drag_debug_log(format!(
                    "side begin controller={controller} generation={generation} origin=({:.3},{:.3})",
                    origin.0, origin.1
                ));
                ...
                .await
                {
                    ...
                    Err(_) => {
                        self.cancel_move_drag_inner()?;
                        Err(
                            "timed out waiting for the side toolbar's hidden drag surface to paint"
                                .to_string(),
                        )
                    }
                }
    ...
    fn start_drag_after_paint(&mut self, generation: u64) -> Result<(), String> {
        ...
        if self.feedback.send(start).is_err() {
            super::super::set_visual_hidden(&self.window, &self.root, GtkToolbarKind::Side, false);
            self.drag.finish();
            return Err(
                "GTK feedback mailbox closed while starting a side-toolbar drag".to_string(),
            );
        }
        ...
        let tick = self.window.add_tick_callback(move |_, _| {
            if intents.send(ViewIntent::Side(DragIntent::Tick { generation })) {
    ...
    fn apply_drag_frame(&mut self, generation: u64) -> Result<(), String> {
        ...
        let (next_x, next_y) = super::super::drag::drag_frame_position(frame.origin, frame.delta);
        let (x, y) = super::super::drag::clamp_drag_offsets(
            &self.window,
            (next_x, next_y),
        ...
        crate::toolbar_gtk::drag_debug_log(format!(
            "side frame generation={generation} seq={sequence} phase={:?} delta=({:.3},{:.3}) origin=({:.3},{:.3}) before=({before_x:.3},{before_y:.3}) preview=({x:.3},{y:.3}) parked_margin=({}, {}) size={}x{}",
            frame.phase,
            frame.delta.0,
            frame.delta.1,
            frame.origin.0,
        ...
        if self
            .feedback
            .send(GtkToolbarFeedback::SetSideOffset {
                x,
                y,
                surface_size: crate::toolbar_gtk::GtkToolbarSurfaceSize::from_window(&self.window),
                seq: sequence,
            ...
            .is_err()
        {
            super::super::set_visual_hidden(&self.window, &self.root, GtkToolbarKind::Side, false);
            self.drag.finish();
            return Err("GTK feedback mailbox closed while moving the side toolbar".to_string());
        }
        ...
        match self.drag.cancel_action() {
            ...
            CancelledDragAction::Finish => {
                ...
                self.feedback
                    ...
                    .map_err(|()| {
                        "GTK feedback mailbox closed while cancelling a side-toolbar drag"
                            .to_string()
                    })
            }
        }
