...
fn assert_replacement(
    ...
    let actual = paint(cache, state, theme, scale);
    ...
    assert_eq!(
        actual,
        paint(&mut RadialBaseCache::default(), state, theme, scale)
    );
    actual
}
...
fn theme_and_density_replacement_matches_fresh_pixels_and_hits_reuse_surfaces() {
    ...
    for scale in [1.0, 2.0] {
        ...
        for theme in [
            ...
            let actual = assert_replacement(&mut cache, &state, &theme, scale);
            assert!(actual.iter().any(|byte| *byte != 0));
            if let Some(previous) = previous {
                assert_ne!(actual, previous, "theme switch must change pixels");
            }
            let surface = cache.cached.as_ref().unwrap().surface.clone();
            assert_eq!(surface.device_scale(), (scale, scale));
            assert_eq!(paint(&mut cache, &state, &theme, scale), actual);
            assert_eq!(
                surface.to_raw_none(),
                cache.cached.as_ref().unwrap().surface.to_raw_none(),
...
fn independent_owners_keep_their_own_surfaces() {
    ...
    let pixels = paint(&mut first, &state, &dark, 1.0);
    let surface = first.cached.as_ref().unwrap().surface.clone();
    paint(&mut second, &state, &light, 1.0);
    assert_ne!(
        surface.to_raw_none(),
        second.cached.as_ref().unwrap().surface.to_raw_none()
    );
    assert_eq!(paint(&mut first, &state, &dark, 1.0), pixels);
    assert_eq!(
        surface.to_raw_none(),
...
fn palette_recents_and_active_changes_replace_rendered_base() {
    ...
    let mut cache = RadialBaseCache::default();
    paint(&mut cache, &state, &theme, 1.0);
    ...
    assert_replacement(&mut cache, &state, &theme, 1.0);
    state
        .style
...
fn bindings_replace_rendered_base() {
    ...
    let mut cache = RadialBaseCache::default();
    let theme = theme::Theme::dark();
    let before = paint(&mut cache, &state, &theme, 1.0);
    let after = assert_replacement(&mut cache, &rebound, &theme, 1.0);
...
fn transparent_swatches_keep_checkerboard_across_themes_and_densities() {
    ...
    for scale in [1.0, 2.0] {
        for theme in [theme::Theme::dark(), theme::Theme::light()] {
            let pixels = paint(&mut cache, &state, &theme, scale);
