...
/// Resting width of one action control. The bar sizes itself from this so a
/// four-control and a five-control bar keep identically sized buttons instead
/// of stretching a fixed width across a varying count.
const ACTION_ITEM_WIDTH: f64 = 74.0;
const BASE_BAR_WIDTH: f64 = BAR_PADDING * 2.0 + ACTION_ITEM_WIDTH * 4.0 + ITEM_GAP * 3.0;
const PIN_BAR_WIDTH: f64 = BAR_PADDING * 2.0 + ACTION_ITEM_WIDTH * 5.0 + ITEM_GAP * 4.0;
...
pub(crate) struct RegionActionBar {
    bounds: RegionActionRect,
    items: [Option<RegionActionItem>; 5],
...
impl RegionActionBar {
    pub(crate) fn place(
        ...
        let natural_width = if pin_available {
            PIN_BAR_WIDTH
        } else {
            BASE_BAR_WIDTH
        };
        let width = natural_width.min((surface_width - SURFACE_MARGIN * 2.0).max(0.0));
        ...
        let item_count = if pin_available { 5.0 } else { 4.0 };
        let item_width =
        ...
        Self {
            bounds,
            items: [
                Some(item(0, RegionAction::Copy)),
                Some(item(1, RegionAction::Save)),
                Some(item(2, RegionAction::Both)),
                Some(item(3, RegionAction::Board)),
                pin_available.then(|| item(4, RegionAction::Pin)),
            ],
            toggle,
        }
    }
...
pub(crate) fn render_region_action_bar(
    ...
    for item in bar.items.into_iter().flatten() {
...
mod tests {
    ...
    fn action_bar_exposes_the_requested_labels_and_shortcuts() {
        ...
        assert_eq!(RegionAction::Pin.label(), "Pin");
        assert_eq!(RegionAction::Pin.shortcut(), "P");
        assert_eq!(
    ...
    #[test]
    fn eligible_pin_reflows_the_review_bar_without_hiding_existing_controls() {
        let bar = RegionActionBar::place(
            RegionSelection {
                start: (100.0, 100.0),
                end: (300.0, 200.0),
        ...
        assert_eq!(bar.bounds(), RegionActionRect::new(8.0, 212.0, 410.0, 88.0));
        for (point, action) in [
            ((53.0, 239.0), RegionAction::Copy),
            ((133.0, 239.0), RegionAction::Save),
            ((213.0, 239.0), RegionAction::Both),
            ((293.0, 239.0), RegionAction::Board),
            ((373.0, 239.0), RegionAction::Pin),
            ((200.0, 279.0), RegionAction::ToggleIncludeDrawings),
        ] {
            assert_eq!(bar.hit(point), Some(action));
        }
    }
}
