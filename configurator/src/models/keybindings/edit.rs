...
pub fn remove_binding(
    ...
    for part in authored_shortcut_parts(&authored) {
        if !removed && KeyBinding::parse(part).is_ok_and(|parsed| parsed == *binding) {
            removed = true;
            continue;
        }
...
fn apply_parsed_text(
    ...
    field: KeybindingField,
    text: &str,
) -> Result<Vec<KeyBinding>, FieldEditError> {
...
pub fn serialize_bindings(bindings: &[KeyBinding]) -> String {
    bindings
        .iter()
...
mod tests {
    ...
    fn equivalent_spelling_cannot_create_a_duplicate_chip() {
        ...
        let binding = KeyBinding::parse("Ctrl+Z").expect("parses");
    ...
    fn removing_one_chip_preserves_siblings_and_authored_spelling() {
        ...
        let binding = KeyBinding::parse("Ctrl+Y").expect("parses");
    ...
    fn invalid_raw_text_blocks_parsed_edits_and_stays_visible() {
        ...
        let binding = KeyBinding::parse("F5").expect("parses");
