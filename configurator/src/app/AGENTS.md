...
## Scope
- Applies to configurator app state, updates, the GTK shell and its pages, effects, daemon setup, search, and session catalog operations.
...
- `component/` owns the GTK shell, view refresh, and effect execution; `pages/` builds the sidebar pages from state.
- `io.rs`, `effects.rs`, `daemon_setup/`, `session_catalog.rs`, `session_catalog/`, and `search/` perform side-effecting or app-wide work.
