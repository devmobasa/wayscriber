...
pub(super) fn perform_daemon_action(
    action: DaemonAction,
    shortcut_input: String,
    paths: &wayscriber::paths::PathResolver,
) -> Result<DaemonActionResult, String> {
    let message = perform_daemon_action_sync(action, shortcut_input.trim(), paths)?;
...
fn perform_daemon_action_sync(
    ...
    match action {
        ...
        DaemonAction::InstallOrUpdateService => {
            ...
            let service_path = install_or_update_user_service(paths)?;
        ...
        DaemonAction::ApplyShortcut => apply_shortcut(shortcut_input, paths),
        DaemonAction::ApplyLightControls => {
            let result = install_hyprland_light_controls(paths)?;
            Ok(result.summary())
        }
    }
}

pub(super) fn load_daemon_runtime_status(
    paths: &wayscriber::paths::PathResolver,
    ...
    let portal_runtime_supported = detect_managed_daemon_portal_runtime_supported(paths);
    let shortcut_backend = ShortcutBackend::from_runtime_inputs(
        desktop,
        read_portal_shortcut_dropin_state(paths),
    ...
    let service_unit_path = detect_service_unit_path(systemctl_available, paths);
    ...
    let configured_shortcut = read_configured_shortcut(shortcut_backend, paths);
    ...
    let light_controls =
        if light_shortcut_apply_capability == LightShortcutApplyCapability::HyprlandNative {
            Some(read_hyprland_light_controls_status(paths))
        } else {
            None
        };
